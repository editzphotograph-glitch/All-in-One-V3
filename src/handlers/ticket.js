...
async function handleTicketOpen(interaction) {
  ...
  try {
    ...
    const sent = await tktChannel.send({ content: `${user.toString()} <@&${MOD_ROLE_ID}>`, embeds: [embed], components: [buttonsRow], allowedMentions: { parse: [], users: [user.id], roles: [MOD_ROLE_ID], } });
